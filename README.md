...
- **改密码**：登录后点顶部「改密码」，输入**原密码**验证身份 + 输入两遍新密码即可，改完立即生效。
...
| 情况 | 行为 |
|---|---|
| **没有**设置环境变量 `ADMIN_PASSWORD` | 密码以**数据库里的值为准**。管理员登录后可在「改密码」页面自行修改，**永久生效**（重启/唤醒不会被覆盖） |
| **设置了**环境变量 `ADMIN_PASSWORD` | 每次应用启动都把 admin 密码校正回该值，数据库里的密码会被覆盖 —— 这是**忘记密码时的找回通道** |

- 默认密码（代码兜底值）：`muyu123`，首次部署时写入数据库。
- **忘记密码怎么办**：在 Render 面板加环境变量 `ADMIN_PASSWORD` = 你想用的密码 → 重启即生效；
  进去后在「改密码」页面改成别的，再把那个环境变量删掉即可。
- 普通用户（非 admin）随时可在顶部「改密码」自助修改，不受上面这条影响。
